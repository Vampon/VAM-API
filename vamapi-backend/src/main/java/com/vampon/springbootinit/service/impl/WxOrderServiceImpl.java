...
import com.vampon.springbootinit.mapper.ProductOrderMapper;
import com.vampon.springbootinit.service.PaymentInfoService;
...
import com.vampon.springbootinit.utils.EmailUtil;
...
import static com.vampon.springbootinit.utils.WxPayUtil.getRequestHeader;

import static com.vampon.springbootinit.constant.PayConstant.*;
import static com.vampon.vamapicommon.model.enums.PayTypeStatusEnum.WX;
import static com.vampon.vamapicommon.model.enums.PaymentStatusEnum.*;
...
public class WxOrderServiceImpl extends ServiceImpl<ProductOrderMapper, ProductOrder> implements ProductOrderService {
    ...
    private void sendPaySuccessEmail(ProductOrder productOrder) {
        ...
        if (StringUtils.isNotBlank(user.getUserEmail())) {
            try {
                new EmailUtil().sendPaySuccessEmail(user.getUserEmail(), mailSender, emailConfig, productOrder.getOrderName(),
                        String.valueOf(new BigDecimal(productOrder.getTotal()).divide(new BigDecimal("100"), 2, RoundingMode.HALF_UP)));
                log.info("发送邮件：{}，成功", user.getUserEmail());
            } catch (Exception e) {
                log.error("发送邮件：{}，失败：{}", user.getUserEmail(), e.getMessage());
            }
        }
    }
