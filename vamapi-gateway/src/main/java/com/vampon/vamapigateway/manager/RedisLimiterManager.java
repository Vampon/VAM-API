package com.vampon.vamapigateway.manager;
...
public class RedisLimiterManager {
     ...
     */
    public void doUserRateLimit(String key) {
     ...
     * @param key 区分不同的限流器，比如不同的用户 id 应该分别统计
     */
    public void doGlobalRateLimit(String key) {
        // 创建一个名称为global_limiter的限流器，每秒最多访问 100 次
        RRateLimiter rateLimiter = redissonClient.getRateLimiter(key);
        /**
         * trySetRate() 方法用于设置速率限制器的速率限制参数。
         * RateType.OVERALL 表示设置速率限制器的整体速率限制，即整个速率限制器受到限制。
         ...
         * RateIntervalUnit.SECONDS 表示速率的时间单位，这里是以秒为单位。
         */
        rateLimiter.trySetRate(RateType.OVERALL, 200, 100, RateIntervalUnit.SECONDS);
        // 每当一个操作来了后，请求一个令牌
        boolean canOp = rateLimiter.tryAcquire(1);
        if (!canOp) {
            throw new BusinessException(ErrorCode.TOO_MANY_REQUEST);
