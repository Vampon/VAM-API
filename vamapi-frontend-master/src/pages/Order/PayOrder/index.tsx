import {Card, message, QRCode, Radio, Spin, Tooltip, Button} from 'antd';
...
} from "@/services/vamapi-backend/orderController";
import {getProductInfoByIdUsingGet} from "@/services/vamapi-backend/productInfoController";
...
const PayOrder: React.FC = () => {
  ...
  const queryOrderStatus = async () => {
    ...
    const getOrder = async () => {
    const productId = BigInt(params.id); // 使用Number构造函数将字符串转换为数字
    const res = await getProductInfoByIdUsingGet({id:productId})
    console.log(res)
    if (res.code === 0 && res.data) {
      const data={
        productInfo:res.data,
