...
import {getInterfaceInfoByIdUsingGet, invokeInterfaceInfoUsingPost} from "@/services/vamapi-backend/interfaceInfoController";
...
// import valueLength from "src/pages/User/UserInfo";
...
const InterfaceInfo: React.FC = () => {
  ...
  const loadedData = async () => {
    ...
    try {
      ...
      if (res.data && res.code === 0) {
        ...
        const subString = res.data?.url.substring(res.data?.url.indexOf("/api"));
        setAxiosCode(axiosExample(res.data?.url, res.data?.method?.toLowerCase()))
        setJavaCode(javaExample(res.data?.url, res.data?.method?.toUpperCase(), res.data?.id, subString))
  ...
  // interface CustomTabType {
  //   key: string;
  //   label: JSX.Element;
  //   tab: string; // Adding tab property to match the CardTabListType
  // }
  // const responseExampleTabList: CustomTabType[] = [
  //   { key: '1', label: <span>Tab 1</span>, tab: 'Tab 1' },
  //   { key: '2', label: <span>Tab 2</span>, tab: 'Tab 2' },
