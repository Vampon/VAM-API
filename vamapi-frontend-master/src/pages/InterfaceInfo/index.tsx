...
const ListOpenApiInfo: React.FC = () => {
  ...
  const [toolsProEditTableDefaultData] = useState<
  ...
  //   const res = await invokeInterfaceInfoUsingPost({ id: data?.id, userRequestParams: requestParams });
  ...
  const submitTools = async () => {
      ...
      setToolsResultLoading(true);
      let requestParams;
      if (data?.method === interfaceMethodList.POST.text) {
        requestParams = JSON.stringify(toolsProEditTableData.reduce((acc, item) => {
          acc[item.paramsName] = item.paramsValue;
          return acc;
        }, {})); // 转换为 JSON 字符串
      } else if (data?.method === interfaceMethodList.GET.text) {
        requestParams = toolsProEditTableData.map(item => `${item.paramsName}=${item.paramsValue}`).join('&'); // 拼接为查询字符串
      } else {
        throw new Error('Unsupported HTTP method');
      }
      // window.alert(`请求参数：${requestParams}`); // 将请求参数显示在警告框中
      const res = await invokeInterfaceInfoUsingPost({ id: data?.id, userRequestParams: requestParams });
